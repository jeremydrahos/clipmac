"""
pystray: systray library
PIL: python image library
ctypes: Windows RegisterHotKey for the global hotkey
threading: allow the tray icon and hotkey listener to run concurrently
...
# Ctrl+Alt+Shift+M via Windows RegisterHotKey (not a keyboard hook).
MOD_ALT = 0x0001
MOD_CONTROL = 0x0002
MOD_SHIFT = 0x0004
MOD_NOREPEAT = 0x4000
...
VK_M = 0x4D
HOTKEY_ID = 1
HOTKEY_MODS = MOD_CONTROL | MOD_ALT | MOD_SHIFT | MOD_NOREPEAT
...
def wake_listener():
    if listener_thread_id:
        user32.PostThreadMessageW(listener_thread_id, WM_QUIT, 0, 0)
...
def request_shutdown(reason):
    ...
    logging.info(f'Exiting via {reason}')
    try:
...
def quit_clicked(icon, item):
    request_shutdown('quit_clicked')

def on_hotkey():
    try:
        root.event_generate('<<ShowMenu>>', when='tail')
    except tk.TclError:
        pass
...
root = tk.Tk()
root.withdraw()
root.bind('<<ShowMenu>>', show_menu)
root.bind('<<QuitApp>>', lambda event: request_shutdown('QuitApp'))
kernel32.SetConsoleCtrlHandler(console_ctrl_handler, True)

def listener_loop():
    global listener_thread_id
    listener_thread_id = kernel32.GetCurrentThreadId()

    if not user32.RegisterHotKey(None, HOTKEY_ID, HOTKEY_MODS, VK_M):
        err = ctypes.get_last_error()
        logging.error(
            f'Failed to register Ctrl+Alt+Shift+M (Win32 error {err}). '
            'Another instance of clipMAC may already own this hotkey.'
        )
        return

    logging.info('Registered hotkey Ctrl+Alt+Shift+M via RegisterHotKey')
    try:
        msg = wintypes.MSG()
        while True:
...
icon_thread = threading.Thread(target=create_systray, name='clipMAC-tray', daemon=True)
listener_thread = threading.Thread(target=listener_loop, name='clipMAC-hotkey', daemon=True)
